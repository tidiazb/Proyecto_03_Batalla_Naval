...
module uart_transmitter
    ...
    (
        ...
        output logic tx_done,             // end of transmission
    ...
    // Registers                    
    logic [1:0] state, next_state;            // state registers
    logic [3:0] tick_reg, tick_next;          // number of ticks received from baud rate generator
    logic [2:0] nbits_reg, nbits_next;        // number of bits transmitted in data state
    logic [DBITS-1:0] data_reg, data_next;    // assembled data word to transmit serially
    logic tx_reg, tx_next;                    // data filter for potential glitches
    
    // Register Logic
    always_ff @(posedge clk_100MHz, posedge reset)
        if(reset) begin
            state <= idle;
            tick_reg <= 0;
            nbits_reg <= 0;
